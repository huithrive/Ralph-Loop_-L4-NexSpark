...
export class MinimumViableAgent {
  private log = createLogger({ context: '[Agent]' });
  ...
  async execute(request: AgentRequest): Promise<{ success: boolean; executionId: string; message: string }> {
    ...
    this.log.info('[Agent] Starting execution', { executionId, userId: request.userId });
    ...
    try {
      ...
      this.log.info('[Agent] Processing request', { executionId, preview: request.request.substring(0, 100) });
      ...
      this.log.info('[Agent] Execution completed successfully', { executionId });
      ...
      this.log.error('[Agent] Execution failed', { executionId, error });
  ...
  async getStatus(executionId: string): Promise<AgentExecution | null> {
    try {
      ...
      this.log.error('[Agent] Error getting status', error);
      return null;
    }
  }
  ...
  private async analyzeRequest(request: AgentRequest): Promise<any> {
    this.log.info('[Agent] Analyzing request with Claude');
...
Keep your response focused and actionable.`;
    ...
    try {
      ...
      this.log.error('[Agent] Claude analysis failed', error);
  ...
  private async generateResponse(request: AgentRequest, analysis: any): Promise<any> {
    this.log.info('[Agent] Generating response with Claude');
...
- Next steps`;
    ...
    try {
      ...
      this.log.error('[Agent] Claude response generation failed', error);
