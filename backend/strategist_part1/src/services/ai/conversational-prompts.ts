 ...
 * All AI prompts and templates for the conversational interview service
 */

/**
 * Acknowledgment generation prompt
 */
export function getAcknowledgmentPrompt(language: 'en' | 'zh'): string {
  return language === 'zh'
...
/**
 * Build acknowledgment user prompt
 */
export function buildAcknowledgmentUserPrompt(
  conversationHistory: string,
  userAnswer: string,
  language: 'en' | 'zh'
): string {
  return `Recent conversation:\n${conversationHistory}\n\nUser just said: "${userAnswer}"\n\nProvide a brief, empathetic acknowledgment (${language === 'zh' ? '3-8个字' : '2-5 words'}).`;
}

/**
 * Next question generation prompt
 */
export function getNextQuestionPrompt(
  language: 'en' | 'zh',
  currentTopic: string,
  userProfile: { brandName?: string; industry?: string; stage?: string }
): string {
  return language === 'zh'
...
当前访谈主题：${currentTopic}
已收集的信息：${JSON.stringify(userProfile, null, 2)}
...
Information gathered: ${JSON.stringify(userProfile, null, 2)}

Based on the conversation history, ask the next most valuable question.`;
}

/**
 * Build next question user prompt
 */
export function buildNextQuestionUserPrompt(conversationHistory: string): string {
  return `Conversation so far:\n${conversationHistory}\n\nWhat's the next insightful question to ask?`;
}

/**
 * Real-time summary generation prompt
 */
export function getRealtimeSummaryPrompt(language: 'en' | 'zh'): string {
  return language === 'zh'
 ...
 */
export function getFallbackAcknowledgment(language: 'en' | 'zh'): string {
  return language === 'zh' ? '好的' : 'Got it';
}

/**
 * Fallback questions
 */
export function getFallbackQuestions(language: 'en' | 'zh'): string[] {
  return language === 'zh'
    ? [
        '能详细说说您的目标客户是谁吗？',
        '您目前最大的增长挑战是什么？',
 ...
 */
export function getDefaultSummary(language: 'en' | 'zh'): {
  industry: string;
  stage: string;
  nextFocus: string;
} {
  return language === 'zh'
    ...
    : {
        industry: 'TBD',
        stage: 'Growth',
        nextFocus: 'Continue conversation'
      };
}
